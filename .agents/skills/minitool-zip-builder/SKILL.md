...
metadata:
  ...
  capabilities_verified_at: "2026-09-16"
  local_revision: "web-api-update"
---
...
目标产物：符合上传规范的 **`.zip` 静态包**。本地页面与端 API 分别验证，SDK 缺失、版本不足、权限拒绝时提供降级；未实测不得声明真机兼容。

## 来源与本地补充

本技能以[官方 1.6.0 包](https://fe-static.xhscdn.com/mini-tool/20260831163932/minitool-zip-builder-1.6.0.skill)为基线，并于 **2026-09-16** 根据[官方能力网页](https://miniapp-sandbox.xiaohongshu.com/minitool/doc)补充端 API。它是仓库维护版，不是未经修改的官方原包，也不能仅凭版本号断言是官方最新发布。

原包的 JS / CSS 兼容规范、性能预算及两份体积审计脚本已纳入。API 差异按当前网页维护：增加 Storage / getLaunchOptions；实况字段使用 `live_photo_sources`；媒体参数不用网络 URL；不把原包中的 `openRedPage`、`tags` 作为当前网页确认支持的能力。`writeTempFile` 统一传完整 data URI，兼容原包更严格的要求。具体参数以本维护版 `references/jsbridge-api.md` 为准；遇到新的官方版本时重新核对，不混用两套字段。
...
3. **Native 能力（JSBridge）** — 需要发笔记、存相册、临时文件或本地缓存时，先读 [jsbridge-api.md](references/jsbridge-api.md)：仅使用文档列出的 `window.xhs.miniTool.*` API，参数与必填项以该文档为准，未列出的字段不要传
...
6. **跨端适配** — 先读 [cross-platform-h5.md](references/cross-platform-h5.md)：触摸、滚动、安全区、PC vs 真机差异；顶部避让与 Logo/标题居中见仓库 [AGENTS.md](../../AGENTS.md)
