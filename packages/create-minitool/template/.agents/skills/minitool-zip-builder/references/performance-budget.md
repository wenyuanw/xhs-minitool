...
3. 大型只读数据集改为摘要、分段样例或适量手工输入；文件选择器仅支持图片／视频，不能承诺导入任意 JSON / CSV 文件。若完整离线数据不可删且仍超门禁，应明确说明该需求不适合小工具，而不是继续打包。
4. 用户产生的数据按版本使用 Storage JS API；只有旧版／未知版本才按需降级浏览器存储，容量与失败处理见 [jsbridge-api.md](./jsbridge-api.md)。不使用任何运行期缓存掩盖巨大的内置种子数据。
...
`FileReader.readAsDataURL()` 可用于用户刚选择的小文件预览或 JSBridge 明确要求 data URI 的短暂转换；大文件不要把结果持久写回静态源码，优先用对象 URL 预览，并在不用时 `URL.revokeObjectURL()`。音视频来源须按容器规则与真机结果验证；播放能力和运行时临时媒体类型不扩展 zip 文件白名单，不把不受支持的音视频扩展名塞进包内。
